  ...
  try {
    ...
    if (req.body.status === "Delivered") {
      ...
      for (const product of order.products) {
        ...
        // Fetch the product details to calculate price
        const productDetails = await Product.findById(productId).select("FinalPrice");

        if (!productDetails) {
          throw new Error(`Product with ID ${productId} not found`);
        }

        const singleProductPrice = productDetails.FinalPrice;
      ...
      res.status(200).json({ order, orderSummaries });
    } else {
      res.status(200).json(order);
    }
  } catch (error) {
...
const getorderbystatus = async (req, res) => {
  try {
    ...
    const orders = await Order.find({ status: status }).sort({ createdAt: -1 }).limit(300).populate({
      path: "products.productId",
      select:
...
const migrateOrdersToOrderSummary = async (req, res) => {
  try {
    ...
    const orders = await Order.find().populate({
      path: "products.productId",
      select: "FinalPrice sallerId", // Ensure necessary fields are selected
    });
    ...
    for (const order of orders) {
      for (const product of order.products) {
        ...
        // Skip if product details are missing
        if (!productId || !productId.FinalPrice) {
          console.log(`Product details missing for order ID: ${order._id}`);
          continue;
        }
        ...
        await OrderSummary.create({
          ...
          totalAmount: totalAmount,
          singleproductprice: singleProductPrice,
          sallerId: productId.sallerId, // Optional field
        });
